...
class FrontendController extends Controller
{
    ...
    public function frontendSections($key ,$status)
    {
        ...
        // dd($status);
        if ($status == 'pending') {
            $content = Frontend::where('data_keys', 'blog' . '.content')->where('status',0)->orderBy('id','desc')->first();
            $elements = Frontend::where('data_keys', 'blog' . '.element')->where('status',0)->orderBy('id')->orderBy('id','desc')->get();
            ...
            $content = Frontend::where('data_keys', 'blog' . '.content')->where('status',1)->orderBy('id','desc')->first();
            $elements = Frontend::where('data_keys', 'blog' . '.element')->where('status',1)->orderBy('id')->orderBy('id','desc')->get();
            $pageTitle = $section->name ;
            $emptyMessage = 'No item create yet.';
            return view('admin.frontend.index', compact('section', 'content', 'elements', 'key', 'pageTitle', 'emptyMessage'));
        }elseif ($status == 'rejected') {
            $content = Frontend::where('data_keys', 'blog' . '.content')->where('status',2)->orderBy('id','desc')->first();
            $elements = Frontend::with('users')->where('data_keys', 'blog' . '.element')->where('status',2)->orderBy('id')->orderBy('id','desc')->get();
            $pageTitle = $section->name ;
            $emptyMessage = 'No item create yet.';
            return view('admin.frontend.index', compact('section', 'content', 'elements', 'key', 'pageTitle', 'emptyMessage'));
        }
        $content = Frontend::where('data_keys', $key . '.content')->orderBy('id','desc')->first();
        $elements = Frontend::with('users')->where('data_keys', $key . '.element')->where('is_submitted','1')->orderBy('id','desc')->get();
        $pageTitle = $section->name ;
        $emptyMessage = 'No item create yet.';
        //dd($elements);
        return view('admin.frontend.index', compact('section', 'content', 'elements', 'key', 'pageTitle', 'emptyMessage'));
            ...
            if ($input_field == 'has_image' && $imgJson) {
                foreach ($imgJson as $imgValKey => $imgJsonVal) {
                    if ($request->id) {
                       ...
                       $validation_rule['image_input.'.$imgValKey] = ['required','image',new FileTypeValidate(['jpg','jpeg','png'])]; 
                    }
    ...
    public function frontendElement($key, $id = null)
    {
        ...
        if ($id) {
            ...
            //$sub_categories = SubCategory::where('category_id',$data->category->id)->get();
            return view('admin.frontend.element', compact('section','edit','categories', 'key', 'pageTitle', 'data'));
        }
        $edit = false;

        return view('admin.frontend.element', compact('section','edit','categories', 'key', 'pageTitle'));
    }
    ...
    public function approve(Request $request)
    {
        ...
        $withdraw = Frontend::where('id',$request->id)->where('status',0)->firstOrFail();
        $withdraw->status = 1;
        ...
        $notify[] = ['success', 'Blog has been approved.'];
        return redirect()->route('admin.frontend.sections',['blog','*'])->withNotify($notify);
    }
    ...
    public function reject(Request $request)
    {
        ...
        $notify[] = ['success', 'Blogs has been rejected.'];
        return redirect()->route('admin.frontend.sections',['blog','*'])->withNotify($notify);
    }
