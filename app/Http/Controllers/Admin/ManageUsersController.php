...
class ManageUsersController extends Controller
{
    public function allUsers()
    {
        ...
        $users = User::select('users.*', DB::raw('SUM(CASE WHEN reward_reviews.status = 1 THEN reward_reviews.total_profit_amount ELSE 0 END) AS total_profit_sum'))
            ->leftJoin('reward_reviews', 'users.id', '=', 'reward_reviews.user_id')
            ->groupBy('users.id')
    ...
    public function detail($id)
    {
        ...
        return view('admin.users.detail', compact('pageTitle', 'user', 'totalReward','totalDeposit', 'totalWithdraw', 'totalTransaction', 'countries', 'totalService', 'totalSoftware', 'totalJob', 'totalServiceBooking', 'totalPurchaseSoftware'));
    }
