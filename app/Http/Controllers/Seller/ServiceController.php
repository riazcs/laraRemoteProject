...
class ServiceController extends Controller
{
    ...
    public function index()
    {
        ...
        $jobs = Job::with(['category','jobBiding'])->where('user_id', $user->id)->latest()->paginate(getPaginate());

       
        
      
        
        return view($this->activeTemplate . 'user.seller.service.index', compact('pageTitle', 'services', 'emptyMessage','softwares','user','section', 'content', 'elements', 'key','jobs'));
    }

    public function create()
    {
    	$pageTitle = "Create service";
        $categories = Category::where('type', 1)->where('status', 1)->orderby('id', 'DESC')->inRandomOrder()->get();
    	$features = Features::latest()->get();
    	return view($this->activeTemplate . 'user.seller.service.create', compact('pageTitle', 'features','categories'));
    }
    ...
    public function store(Request $request)
    {
        ...
        $request->validate([
            'image' => ['required', 'image', new FileTypeValidate(['jpg', 'jpeg', 'png']),'dimensions:min_width=920,min_height=468'],
            ...
            'image.dimensions' => 'Image dimension should be 920X468 or higher.',
        ]);
        $service = new Service();
    ...
    public function edit($id, $slug)
    {
        ...
        return view($this->activeTemplate . 'user.seller.service.edit', compact('pageTitle', 'features', 'service','categories'));
    }
